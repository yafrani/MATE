...
for instance in n:
    for jump in m:
        ...
        for mut_rate in vec_rate:
            vec_iter = [ceil(size**jump), ceil(exp(1)*(size**jump)), ceil(2*exp(1)*(size**jump))] 
            for nb_iter in vec_iter:
                for run in range(0, nb_run):
                    nb_hit=nb_iter+1
                    print("Instance: jump", str(instance),str(jump), str(mut_rate), str(nb_iter), str(run))
                    ...
                    result_line = 'Jump' + ';' + str(instance)+ ';' + str(jump)+ ';'+ str(nb_iter)+ ';' + 'EA'+ ';' + str(mut_rate)+';' + str(run) + ';' + str(fsol1)+ ';' + str(nb_hit)
                    #print (result_line)
                    # save result
                    fmet = open('output-Jump-EA.csv', 'a')
